 ...
 */
export function exportInteractionTree(rootNodeId, nodes, edges) {
    const errors = [];
     ...
     */
    function resolveEdgeType(e) {
        const dt = e.data?.edgeType;
        if (typeof dt === 'string')
            return dt;
     ...
     */
    function buildNode(nodeId, ancestors = new Set()) {
        ...
        if (replaceEdges.length > 0) {
            ...
            const defaultValueExtras = (typeof rawDefaultValue === 'object' && rawDefaultValue !== null && !Array.isArray(rawDefaultValue)
                ? { ...rawDefaultValue }
                : {});
            ...
            if (typeof rawDefaultValue === 'object' && rawDefaultValue !== null && !Array.isArray(rawDefaultValue)) {
                const defaultValueObject = rawDefaultValue;
    ...
    const rootNode = nodeMap.get(rootNodeId);
    const rootData = rootNode?.data;
    const rootRaw = rootData?.rawFields ?? {};
    const rootResult = { ...rootRaw };
